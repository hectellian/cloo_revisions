...
Une relation *is-a* entre 2 classes est **necessaire** mais pas **suffisante** pour que l'utilisation d'heritage soit coherante et sensee
...
- **Pre-conditions** : Contraintes qui doivent etre realisees *avant* d'effectier l'operation
...
- **Post-conditions** : Contraintes qui doivent etre realise *apres* avoir effectue l'operation
...
- Si **S** est une sous classe de **T** $\rightarrow$ on doit pourvoir utiliser une instasnce **S** chaque fois qu'une instance de **T** est attendue, *sans causer de problemes*
...
Pour tout **sous-types** (PECS):

- Les types des args peuvent etre generalises, mais pas specialises $\rightarrow$ `<? super ...>` *contravariance*
- Les types de retour peuvent etre specialises, mais pas generalises $\rightarrow$ `<? extends ...>`*covariance*
- Si un type apparait en args et en retour, on doit le preserver tel quel $\rightarrow$ `<T>` *invariance*
